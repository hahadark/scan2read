...
- 진행률은 파일 개수가 아니라 **OCR한 페이지 수**로 집계한다(`_update_batch_progress`).
  큰 책 한 권만 큐에 있으면 파일 개수 기준 진행바는 끝까지 0%였기 때문이다. 모든 파일의
  페이지 수를 알 때만 페이지 기준으로 계산하고, 하나라도 모르면 기존처럼 파일 개수로
  물러난다. 페이지를 다 읽었지만 아직 끝나지 않은 구간은 "OCR 100%"가 아니라
  "문장 정리·EPUB 만드는 중…"으로 표시한다 — 그 뒤에 재구성·AI 보정·EPUB 생성이 남아 있어서다.
- "EPUB 편집" 탭은 자체적으로 제공자·모델 콤보(AI 설정 탭과 같은 변수를 공유), 연결 확인
  버튼, API 상태, 모델 요금, 파일 선택 직후 계산되는 예상 사용량(문단 수·글자 수·예상 최대
  비용), 진행률, 실제 사용 토큰·비용을 함께 보여준다. 문단 수 계산은 GUI 프로세스가 아니라
  `inspect-epub` 서브프로세스가 한다 — 미리보기의 블록 인덱스는 나중에 그것을 적용할 코드와
  **같은 런타임**에서 나와야 하기 때문이다(프리징된 GUI 사본과 `app/scan2read`가 어긋날 수
  있으므로). EPUB 파일은 드래그 앤 드롭으로도 넣을 수 있다(확장자로 PDF 대기열과 구분).
...
2026-09-15 현재 322개 통과, 실제 외부 OCR 환경 테스트 1개는 환경변수가 없으면 건너뛴다.
