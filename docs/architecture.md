...
- `ai_enhance.py`: `AIOptions` 데이터클래스 불리언 필드 하나가 기능 하나(문단 경계, OCR
  의심 단어, 띄어쓰기, 이상 문자, 구조 분류, 목차, 괄호·음역 삭제, 도식 잔재 삭제)에
  대응한다. 이 중 "도식 잔재 삭제"만 문단 전체를 비우며(EPUB 생성이 빈 텍스트를 건너뛴다),
  AI 판정에 더해 `looks_like_figure_residue()`를 통과해야 한다 — 같은 함수를 `ai_filter`가
  "보낼 가치가 있나"에, `ai_enhance`가 "지워도 되나"에 쓰므로 AI가 진짜 본문을 잔재로
  잘못 판정해도 삭제로 이어질 수 없다.
  ...
  `custom_rule`(문자열, 최대 300자)만 예외로 토글이 아니라 자유 텍스트다 — 켜진
  기능들의 요청 `instructions`에 마지막 문장으로 덧붙을 뿐 자체 스키마 속성은
  없으므로, 최소 하나의 다른 기능이 켜져 있지 않으면 아무 효과가 없다. 적용
  안전장치를 우회할 수는 없다(모델이 어떤 규칙을 받든 `_apply_edits`/
  `_apply_glosses`의 검증을 반드시 통과해야 실제로 반영된다). 문단 경계 판정
  (`ai_context.py`)에는 적용되지 않는다.
- `ai_cache.py`: 문단·모델·기능 조합(사용자 지정 규칙 문구 포함)을 여러 책·재실행에
  걸쳐 캐시 — 규칙 문구를 바꾸면 그 조합의 캐시 키도 바뀌어 이전 규칙으로 계산된
  결과를 재사용하지 않는다.
...
## EPUB 규칙 편집 (PDF 변환과 별개 경로)

이미 만들어진 EPUB을 자연어 규칙 하나로 고치는 기능은 변환 파이프라인을 전혀 거치지
않는다. 안전 모델부터 반대다 — `ai_enhance.py`는 OCR 손상 복구가 목적이라 모델이
산문을 못 고치게 막지만, 여기서는 사용자가 "빼줘"라고 *요청*하는 것이므로 교체·삭제를
그대로 허용한다. 대신 안전장치가 다른 곳에 있다.

- `epub/editor.py`: 표준 라이브러리(zipfile + ElementTree)만으로 EPUB을 읽고 쓴다.
  OPF의 spine 순서로 본문 문서를 찾고, 그 안에서 블록 태그 중 **자손에 다른 블록
  태그가 없는 것**만 편집 단위(`Block`)로 잡는다. 쓸 때는 편집 대상이 아닌 zip 멤버를
  전부 바이트 그대로 복사하고, 편집 대상 문서 안에서도 실제로 바뀐 요소만 손댄다
  (그래서 안 바뀐 문단의 인라인 서식은 보존된다). XHTML에 흔한 `&nbsp;` 같은 HTML
  엔티티는 XML 파서가 거부하므로 파싱 전에 문자로 치환한다.
- `cleanup/ai_edit.py`: 규칙과 문단을 받아 `keep`/`replace`/`delete`를 제안만 한다.
  **파일을 절대 쓰지 않는다.**
- 적용은 완전히 분리된 두 번째 단계다. `cli.py`의 `edit-epub`은 `--plan`(제안을 JSON으로
  쓰고 종료, API 호출 있음)과 `--apply-plan`(JSON을 적용해 새 EPUB 생성, API 호출 없음)로
  나뉘고, GUI가 그 사이에 사람이 보는 미리보기를 끼워 넣는다. 원본 EPUB에 쓰는 경로는
  아예 존재하지 않는다(`--output`이 원본과 같으면 거부).
- 사전 필터가 없다 — 규칙이 어느 문단에 걸릴지 로컬에서 알 수 없으므로 전 문단을 보낸다.
  비용은 기존 책당 한도(`AICostBudget`)로 막고, 한도에 걸리면 거기까지의 제안만 남긴다.
